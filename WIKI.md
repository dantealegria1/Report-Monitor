...
## Table of Contents
1. [Overview](#overview)
2. [KPIs (Key Performance Indicators)](#kpis-key-performance-indicators)
3. [Filters](#filters)
4. [Charts and Visualizations](#charts-and-visualizations)
5. [Anomaly Detection](#anomaly-detection)
6. [Drift Analysis](#drift-analysis)
...
### 1. Total Ejecuciones (Total Executions)
...
### 2. Duracion Promedio (Average Duration - Success Only)
...
### 4. Variabilidad (MAD mediana) - Variability
...
### 1. Tendencia de Ejecuciones (Execution Trends)
...
### 2. Top 10 Reportes más Lentos (Top 10 Slowest Reports)
...
### 3. Top 10 Reportes con más Fallos (Top 10 Reports with Most Failures)
...
### 4. Distribución de Duración (Duration Distribution)
...
### 5. Saturación por Hora (Hourly Saturation)
...
### 6. Heatmap Hora vs Día de semana (Hour vs Weekday Heatmap)
...
When anomalies are detected, you'll see a table with:
- **started_at**: When the execution started
- **ReportType**: Category of the report
- **ReportName**: Specific report name
- **hour**: Hour of day (0-23)
- **duration_seconds**: How long it took
- **execution_status**: Should be "success" (only successful runs are analyzed)
- **z_robusto**: The robust Z-score (higher = more anomalous)

**What to do with anomalies**:
1. Investigate reports with highest z_robusto scores first
2. Check if there were system issues at that time
3. Look for patterns (same report, same time of day, etc.)
4. Consider if the report needs optimization
...
#### 1. Por ReportType (By Report Type)
- Analyzes drift at the category level
- Good for understanding overall trends
- Requires less data

#### 2. Por ReportName (Top 50)
- Analyzes drift for individual reports
- More granular insights
- Only looks at top 50 most frequent reports
...
A_p50: 45s    B_p50: 30s    delta_p50: +15s
A_p90: 90s    B_p90: 60s    ratio_p90: 1.5
```
**Interpretation**: Financial reports are getting slower - median increased by 15s, and the 90th percentile is 1.5× slower than before. Investigation needed!
...
1. **Ventana actual** (Current Window):
...
- `CACHE_TTL`: How long to cache data (default: 600 seconds)
- `MAX_DURATION_HOURS`: Maximum reasonable duration (default: 6 hours)
- `TOP_REPORTS_LIMIT`: Number of reports in filter dropdown (default: 200)
